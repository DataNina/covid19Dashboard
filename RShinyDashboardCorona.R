...
setwd("C:/Users/Nina/Documents/R/Corona")

#Load the data
load(file="Covid19Data")


#Load libraries
necessaryPackages <- c("shiny", "shinydashboard", "extrafont", "ggplot2", "lubridate")
suppressMessages(suppressWarnings(invisible(lapply(necessaryPackages, library, character.only = TRUE))))
...
#Set options for accessing shiny dashbaord
options(shiny.host="0.0.0.0")
options(shiny.port=6483)

#Set default language to English
Sys.setlocale("LC_TIME", "English")


#If dashboard was alredy run, just run the dashboard
if ((lubridate::date(Sys.time())-1)==max(covid19data$date)) {
  requiredPackages <- c("purrr", "plyr", "dplyr")
  suppressMessages(suppressWarnings(invisible(lapply(requiredPackages, library, character.only = TRUE))))
} else {
  rm(covid19data)
...
output$rplot <- renderPlot ({
  ...
  suppressWarnings(print(myplot2))
},
...
output$iplot <- renderPlot ({
  ...
  suppressWarnings(print(myplot3))
},
